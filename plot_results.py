...
from flowcyt.evaluation import shorten
...
if TARGET_COL == "NPM":
    # For NPM
    expnames = [
        "age, sex, blast percentage_predict-NPM_stamp-20250625_161814",
        "huaspossible_predict-NPM_configname-huaspossiblenotab_nparams-726_stamp-20260108_175557",
        "huaspossible_predict-NPM_configname-huaspossible_nparams-738_stamp-20260108_173241",
        "cell-level-model_predict-NPM_ncells-10000_withtab-False_clf-decisiontreeclassifier_pooling-max_stamp-20260114_114900",
        "cell-level-model_predict-NPM_ncells-10000_withtab-True_clf-decisiontreeclassifier_pooling-max_stamp-20260114_144605"
    ]
...
f, ax = plt.subplots()
model_1 = preds_table.columns[0]
model_2 = preds_table.columns[1]
g = sns.scatterplot(
    preds_table,
    x=model_1,
    y=model_2,
...
def annotate(data, **kws):
    r, p = pearsonr(data[model_1], data[model_2])
    ax = plt.gca()
    ax.text(.6, .6, 'r = {:.2f}, $P$ = {:.2g}'.format(r, p),
            transform=ax.transAxes)

ax.plot([0, 1], [0, 1], color="gray", linestyle="--")
annotate(preds_table)
ax.set_xlim(0, 1)
ax.set_ylim(0, 1)
ax.set_xlabel(f"{model_1} - " + "$\\hat{P}(Y=1)$")
ax.set_ylabel(f"{model_2} - " + "$\\hat{P}(Y=1)$")
ax.set_title("Comparison of classifiers output probabilities")
ax.legend(loc="lower right", labels=["FLT3-ITD", "WT"], title="Ground truth")
f.savefig("/home/jonalegr/Documents/mutation_status_prediction/classifiers-probas.png")
...
# Generate ROC curves for all left out folds
# We need to sort the experiments in the order of the `new_labels`

def sort_exps(values):
    def key_generator():
        for val in values:
            yield expnames.index(val)
    return tuple(key_generator())

sorted_classifications = classifications.sort_values(by="expname", key=sort_exps)

from sklearn.metrics import RocCurveDisplay
fig, ax = plt.subplots(figsize=(8, 8))
plot_chance = False
for i, (expname, subdf) in enumerate(sorted_classifications.groupby("expname", sort=False)):
    print(expname)
    y_true = subdf.loc[:, "y_true"].values.astype(int)
    y_score = subdf.loc[:, "y_score"].values
    if i + 1 == len(expnames):
        plot_chance = True
    RocCurveDisplay.from_predictions(
        ...
        name=new_labels[i],
        ax=ax,
        plot_chance_level=plot_chance
    )
plt.title("ROC curves\nAggregated results on left-out folds")
plt.savefig("plots/roc_curves.svg")
